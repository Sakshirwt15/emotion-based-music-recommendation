...
st.markdown(
    """
    <style>
        .main-title {text-align: center; font-size: 38px; font-weight: 700; color: #1DB954; margin-bottom: 6px;}
        .sub-title {text-align: center; font-size: 16px; color: #bdbdbd; margin-bottom: 22px;}
        .emotion-box {background-color: transparent; padding: 12px; text-align: center; font-size: 20px; font-weight: 700; color: white; margin: 8px 0 14px 0;}
        .song-card {background-color: white; padding: 12px 16px; border-radius: 12px; margin: 10px 0 18px 0; box-shadow: 0px 2px 6px rgba(0,0,0,0.2);}
        .song-title {color: #000; font-weight: 700; font-size: 18px;}
        .song-artist {color: #444; font-style: italic; font-size: 16px;}
        .footer {text-align: center; margin-top: 32px; font-size: 13px; color: gray;}
    </style>
    """,
...
def recommend_songs(emotion_list):
    data = pd.DataFrame()
    times_dict = {1:[30], 2:[30,20], 3:[55,20,15], 4:[30,29,18,9], 5:[10,7,6,5,2]}
...
face_cascade = load_haar(haar_path)
...
if st.button("📸 Start Emotion Scan"):
    ...
    while count < 20:  # sample 20 frames
        ...
        for x,y,w,h in faces:
            ...
            cv2.putText(frame, emotion_dict[max_index], (x,y-10), cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,255),2)
...
if st.session_state["emotions_detected"]:
    st.markdown(f"<div class='emotion-box'>Detected Emotions: {', '.join(st.session_state['emotions_detected'])}</div>", unsafe_allow_html=True)
        ...
        for i,s in enumerate(rows, start=1):
            ...
            if s["spotify_id"]:
                components.iframe(
                    ...
                    height=80, width=700, scrolling=False
                )
            else:
